...
async function generateGalleries() {
  try {
    ...
    const jsonPath = path.join(process.cwd(), "..", "front", "data", "imagesData.json")
...
`
    ...
    images.forEach(img => {
      ...
      let variants = `
        <div class="gallery-variant">
          Voir en :
          <a href="${img.original_url.replace('upload/', 'upload/f_avif/')}" target="_blank">AVIF</a> |
          <a href="${img.original_url.replace('upload/', 'upload/f_jpg/')}" target="_blank">JPG</a> |
          <a href="${img.original_url.replace('upload/', 'upload/f_png/')}" target="_blank">PNG</a>
        </div>
      `
      md += `
  <div class="gallery-card">
    ...
    <div class="cloud-info">
      Format : <b>${img.format || "-"}</b> | Dimensions : <b>${img.width || "?"}×${img.height || "?"}</b> | Taille : <b>${humanFileSize(img.bytes || 0)}</b>
    </div>
    <div class="cloud-tags">Tags : ${(img.tags && img.tags.length) ? img.tags.join(", ") : "aucun"}</div>
